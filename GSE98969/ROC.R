...
load('/share/home/duanshumin/wilsonmak/analysis/AD_microglia/GSE98969/20181115/GSE98969_stabs_selected_genes.Rdata')

set.seed(2018)
randomly_select_genes1 <- sample(colnames(learnCV), 48)
randomly_select_genes1 <- which(colnames(learnCV[-1]) %in% randomly_select_genes1)

set.seed(2019)
randomly_select_genes2 <- sample(colnames(learnCV), 48)
randomly_select_genes2 <- which(colnames(learnCV[-1]) %in% randomly_select_genes2)

set.seed(2020)
randomly_select_genes3 <- sample(colnames(learnCV), 48)
...
# print(p)
# dev.off()
save(p, acc, auc, file='performance_GSE98969.Rdata')
