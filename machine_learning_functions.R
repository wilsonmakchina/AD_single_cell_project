enet_CV <- function(data, n_folds, model_name, cores, seed) {
    ...
    search_all <- foreach(k = 1:n_folds, .combine = rbind) %do% {        
        ...
        # stability selection
        set.seed(seed)
        stab <- stabs::stabsel(x = train_x, y = train_y, fitfun = model, args.fitfun = list(family = "binomial"), cutoff = 0.6, PFER=1, papply = parallel::mclapply, mc.cores = 10, sampling.type = "SS")
