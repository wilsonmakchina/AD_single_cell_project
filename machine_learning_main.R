...
mydata <- t(normalized_data@data)

# extract var.genes
mydata <- mydata[,normalized_data@var.genes]

metadata <- data.frame(normalized_data@ident, cell='MG')
colnames(metadata)[1]='cell_type'
metadata$cell_type <- as.character(metadata$cell_type)

metadataCtrlAD <- metadata[metadata$cell_type == ' Resting MG' | metadata$cell_type == " DAM",]
...
set.seed(2018)
stab <- stabs::stabsel(x = learnCV[-1], y = learnCV[,1], fitfun = 'glmnet.enet_0.2', args.fitfun = list(family = "binomial"), cutoff = 0.9, PFER=1, papply = parallel::mclapply, mc.cores = 35, sampling.type = "SS")
stab$selected][2]
...
set.seed(2018)
svm <- train(learnCV[-1][svm_rfe_opt_variables], learnCV[,1], method = "svmLinear",trControl = trainControl(method='none'))
